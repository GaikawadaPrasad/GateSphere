...
- Services mix in `UnitScopedAccess`: `_assert_unit_visible(unit_id)` on every detail/mutation
  ...
  `deliveries`, `amenities` (bookings only — amenities/slots/rules stay community‑wide).
- Consequences: a resident sees only their unit's visitor requests / deliveries / invoices /
  ledger / tickets and their own amenity bookings; cannot raise invoices (`STAFF_ONLY`), decide
  another unit's delivery, or post internal ticket notes; internal notes are hidden from
  residents even on their own tickets.
...
| **communication** (FR‑12) | Notices, emergency alerts, polls, surveys, events. Target = specific towers / blocks / resident groups / whole community (`announcement_targets`). **Validate the initiator is authorized to broadcast to the selected scope before publish.** Published broadcasts are a permanent record. One poll response per user unless repeat voting is explicitly enabled. A `survey` announcement carries one poll per question (`GET .../survey` aggregates them); an `event` announcement accepts per-user RSVPs (`event_rsvps`: going/maybe/not_going + guests). |
...
| **audit / search / reports** (FR‑16) — `?q=` free-text on visitors/staff/vehicles/complaints/incidents/deliveries; `.csv` export on audit/invoices/payments/gate-events/visitor-entries/tickets (`app/core/export.py`, `<module>:export`) | `audit_logs`: **append‑only, enforced by a DB trigger** (`gs_audit_logs_immutable`, migration 0028 — RAISEs on any UPDATE/DELETE) — `community_id`, `user_id`, `session_id`, `role_slug` (point‑in‑time), `action`, `module`, `entity_type`, `entity_id`, `old_values`, `new_values`, `ip_address`, `user_agent`, `created_at`. Search/filter/report results have role + community scope applied **before** query execution. Auditor endpoints GET‑only. |
