...
| **audit / search / reports** (FR‑16) | `audit_logs`: **append‑only, enforced by a DB trigger** (`gs_audit_logs_immutable`, migration 0028 — RAISEs on any UPDATE/DELETE) — `community_id`, `user_id`, `session_id`, `role_slug` (point‑in‑time), `action`, `module`, `entity_type`, `entity_id`, `old_values`, `new_values`, `ip_address`, `user_agent`, `created_at`. Search/filter/report results have role + community scope applied **before** query execution. Auditor endpoints GET‑only. |
