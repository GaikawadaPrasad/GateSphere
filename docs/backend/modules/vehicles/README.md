# Module: Vehicle, Parking & Violations

> Canonical spec for the `vehicles` module. Behaviour-changing code MUST update this file in the same PR.

## Purpose
Resident/visitor vehicles, parking-slot allocation, entry/exit logging, unauthorized-parking violations.
...
## API
Base path: `/api/v1/vehicles`. Contract: [`docs/backend/api/vehicles.md`](../../api/vehicles.md); OpenAPI at `/docs`.

## Events
- Emails / notifications: _list_
- Background jobs (Celery): `app/modules/vehicles/tasks.py`
- Audit events: _list_
