...
> Canonical spec for the `users` module. Behaviour-changing code MUST update this file in the same PR.

## Purpose
User accounts, roles, permissions, role-permission matrix, per-community role assignment.
...
## API
Base path: `/api/v1/users`. Contract: [`docs/backend/api/users.md`](../../api/users.md); OpenAPI at `/docs`.

## Events
- Emails / notifications: _list_
- Background jobs (Celery): `app/modules/users/tasks.py`
- Audit events: _list_
