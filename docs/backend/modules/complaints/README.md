# Module: Complaint & Service Desk

> Canonical spec for the `complaints` module. Behaviour-changing code MUST update this file in the same PR.

## Purpose
SLA-driven ticket lifecycle, priorities, technician assignment, escalation, feedback ratings.
...
## API
Base path: `/api/v1/complaints`. Contract: [`docs/backend/api/complaints.md`](../../api/complaints.md); OpenAPI at `/docs`.

## Events
- Emails / notifications: _list_
- Background jobs (Celery): `app/modules/complaints/tasks.py`
- Audit events: _list_
