...
> Canonical spec for the `auth` module. Behaviour-changing code MUST update this file in the same PR.

## Purpose
Session-cookie login/logout, password hashing (Argon2), CSRF, session lifecycle.
...
## API
Base path: `/api/v1/auth`. Contract: [`docs/backend/api/auth.md`](../../api/auth.md); OpenAPI at `/docs`.

## Events
- Emails / notifications: _list_
- Background jobs (Celery): `app/modules/auth/tasks.py`
- Audit events: _list_
