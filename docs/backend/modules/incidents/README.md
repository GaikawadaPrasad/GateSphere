# Module: Emergency & Incident Management

> Canonical spec for the `incidents` module. Behaviour-changing code MUST update this file in the same PR.

## Purpose
Log medical/fire/theft/breach incidents, severity, assignment, actions taken, resolution audit trail.
...
## API
Base path: `/api/v1/incidents`. Contract: [`docs/backend/api/incidents.md`](../../api/incidents.md); OpenAPI at `/docs`.

## Events
- Emails / notifications: _list_
- Background jobs (Celery): `app/modules/incidents/tasks.py`
- Audit events: _list_
