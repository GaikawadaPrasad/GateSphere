...
| `GET /communication/announcements` | `communication:view` | – | `200` list | `?status=all\|published\|draft\|expired` (admin tabs; `published` = published **and** not expired, `draft` = unpublished and not expired); without `status`, legacy `?published_only=` (default true) applies. `?community_id=`. `?category=` / `?exclude_category=` (general\|maintenance\|security\|amenities\|billing\|events; invalid → `422 INVALID_ENUM`) — resident Maintenance tab = `category=maintenance`, Community Notices = `exclude_category=maintenance`. **Unit-restricted viewers (residents) only ever get published rows targeted at them, whatever filter they send.** |
| `POST /communication/announcements` | `communication:create` | `AnnouncementCreate` | `201` single | draft; targets validated against the community; `category` (default `general`) |
