...
export default function FinancialSummaryPage() {
  ...
  const outstandingInvoices = (invoices || []).filter(
    (inv) => inv.status === "overdue" || (inv.status === "posted" && parseFloat(inv.balance_due || "0") > 0)
  );
  ...
  const invoiceColumns: Column<MaintenanceInvoice>[] = [
    ...
    },
    {
      ...
      render: (item) => {
        ...
        return (
          <span style={{ color: bal > 0 ? "#dc2626" : "var(--fg)", fontWeight: bal > 0 ? 600 : 400 }}>
            {formatCurrency(bal)}
          </span>
        );
      },
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1rem" }}>
            <div className="card" style={{ borderTop: "3px solid #3b82f6" }}>
              <div style={{ fontSize: "0.8rem", color: "var(--muted)", fontWeight: 600 }}>TOTAL INVOICED REVENUE</div>
              <div style={{ fontSize: "1.75rem", fontWeight: 700, color: "var(--fg)", margin: "0.4rem 0" }}>
                {formatCurrency(parseFloat(overview?.totalBilled || "0"))}
              </div>
              <div style={{ fontSize: "0.75rem", color: "var(--muted)" }}>Gross billings generated across all units</div>
            </div>

            <div className="card" style={{ borderTop: "3px solid #10b981" }}>
              <div style={{ fontSize: "0.8rem", color: "var(--muted)", fontWeight: 600 }}>TOTAL COLLECTED</div>
              <div style={{ fontSize: "1.75rem", fontWeight: 700, color: "#059669", margin: "0.4rem 0" }}>
                {formatCurrency(parseFloat(overview?.totalCollected || "0"))}
              </div>
              <div style={{ fontSize: "0.75rem", color: "var(--muted)" }}>Reconciled & verified payments</div>
            </div>

            <div className="card" style={{ borderTop: "3px solid #ef4444" }}>
              <div style={{ fontSize: "0.8rem", color: "var(--muted)", fontWeight: 600 }}>TOTAL OUTSTANDING DUES</div>
              ...
              <div style={{ fontSize: "0.75rem", color: "var(--muted)" }}>Unpaid and overdue balances</div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem" }}>
              {Object.entries(audit?.payment_methods_breakdown || {}).map(([method, data]) => (
                ...
                >
                  <div style={{ fontSize: "0.75rem", color: "var(--muted)", textTransform: "uppercase" }}>{method}</div>
                  <div style={{ fontSize: "1.35rem", fontWeight: 700, color: "#059669", margin: "0.25rem 0" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "1.5rem" }}>
          ...
          <div className="card">
            ...
            {rules ? (() => {
              ...
              if (!activeRule) return <div style={{ color: "var(--muted)", fontSize: "0.85rem" }}>No custom billing rules configured.</div>;
              return (
                <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", fontSize: "0.85rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", padding: "0.5rem 0", borderBottom: "1px solid var(--border-light)" }}>
                    ...
                    <span style={{ fontWeight: 600, textTransform: "capitalize" }}>{activeRule.billing_frequency || "Monthly"}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", padding: "0.5rem 0", borderBottom: "1px solid var(--border-light)" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", padding: "0.5rem 0", borderBottom: "1px solid var(--border-light)" }}>
                    ...
                    <span style={{ fontWeight: 600 }}>{activeRule.grace_period_days || 5} days</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", padding: "0.5rem 0" }}>
                    <span style={{ color: "var(--muted)" }}>Late Fee</span>
                    <span style={{ fontWeight: 600 }}>
                      {activeRule.late_fee_type === "percentage" ? `${activeRule.late_fee_amount}%` : formatCurrency(parseFloat(activeRule.late_fee_amount || "0"))}
                    </span>
                  </div>
                </div>
              );
            })() : (
              <div style={{ color: "var(--muted)", fontSize: "0.85rem" }}>No custom billing rules configured.</div>
            )}
          </div>
