...
export default function SecuritySupervisorAuditLogsPage() {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const gates =
        gatesRes.status === "fulfilled" && Array.isArray(gatesRes.value) ? gatesRes.value : [];
      ...
      const mappedLogs: DisplayAuditLog[] = events.map((e: any) => {
        const isOverride =
          e.event_type === "checkpoint_override" || e.event_type?.includes("override");
        ...
        const detailInfo =
          e.metadata?.reason ||
          e.metadata?.notes ||
          (isOverride ? "Supervisor override executed" : "Standard verification logged");
        ...
        >
          <div>
            ...
            <p className="card-subtitle">
              Server-verified append-only audit records for all gate checkpoints and security events
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            <div style={{ padding: "40px", textAlign: "center", color: "var(--muted)" }}>
              ...
              <p style={{ fontSize: "0.85rem" }}>
                Gate overrides and checkpoint transactions will automatically appear here.
              </p>
            </div>
          ) : (
