...
- `(public)/` — login (built), forgot-password, reset-password.
- `(protected)/` — `layout.tsx` (auth shell: re-validates `/auth/me`, redirects if signed out) +
  all module routes (dashboard built; communities, towers, floors, units, residents, visitors,
  gate, deliveries, domestic-staff, vehicles, parking, violations, billing, payments, complaints,
  amenities, communications, notifications, incidents, reports, audit-logs, profile).
- `/unauthorized` — the 403 / access-denied screen (a real path, not a route group — `(x)/page`
  would collide with `/`).
- `lib/` (`api.ts`, `query.ts`, `permissions.ts`), `hooks/` (`use-auth.ts`), `store/` (`ui.ts`,
  Zustand). Add `components/{ui,<module>}/`, `features/`, `types/` as modules land.
...
(= `gs_csrf` cookie) on unsafe methods. Unwraps the `{success,message,data,meta}` envelope:
`apiGet` / `apiSend` return `data`; `apiList` returns `{items, meta}`. Throws
`ApiError { status, code, message, fields }` on `success:false` or non-2xx — pages branch on
`err.isNotFound` / `isForbidden` / `isUnauthenticated` and map `err.fields` onto the form.
One typed client object per module (see `authApi`). No `any`.
