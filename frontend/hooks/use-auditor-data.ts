  ...
  return useQuery({
    ...
    queryFn: async () => {
      const fetchOverview = communityId
        ...
        : Promise.resolve({});
      const fetchFinancial = communityId
      ...
      // Compute financial reconciliation dynamically from real financial data
      let financialReconciliationPct = 100;
      if (financial && Number(financial.total_billed) > 0) {
        financialReconciliationPct =
          Math.round(
            (Number(financial.total_collected || 0) / Number(financial.total_billed)) * 1000,
          ) / 10;
        ...
        financialReconciliationPct =
          totalBilled > 0 ? Math.round((totalPaid / totalBilled) * 1000) / 10 : 100;
      }

      return {
        open_audit_events: Number(
          overview?.open_tickets ?? overview?.open_tickets_count ?? totalLogs,
        ),
        total_actions_today: totalLogs,
        flagged_anomalies: gateAnomalyCount,
        active_user_sessions: Number(overview?.residents ?? overview?.active_residents_count ?? 0),
        financial_reconciled_pct: financialReconciliationPct,
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.map((inc: any) => ({
        id: inc.id,
        incident_number: inc.incident_number || `INC-${inc.id.slice(0, 6)}`,
