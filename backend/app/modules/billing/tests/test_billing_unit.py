...
async def _invoice(svc, unit, **kw):
...
async def test_invoice_totals_with_tax(db, scope_for, community, unit, superadmin):
    svc = _svc(db, scope_for(community.id), superadmin)
    await svc.update_rule(schemas.RuleUpdate(tax_percent="10"), community_id=community.id)
...
async def test_post_then_pay_updates_status_and_ledger(db, scope_for, community, unit, superadmin):
    svc = _svc(db, scope_for(community.id), superadmin)
    await svc.update_rule(schemas.RuleUpdate(tax_percent="0"), community_id=community.id)
    inv = await _invoice(svc, unit)
    await svc.post_invoice(inv.id)
    assert inv.status == "posted"

    # partial payment
    pay1 = await svc.record_payment(
    ...
    rows, _ = await svc.unit_ledger(unit.id, offset=0, limit=20)
    assert rows[0].balance_after == Decimal("0.00")  # newest first
    assert pay1.payment_reference.startswith("PAY-")


async def test_over_allocation_rejected(db, scope_for, community, unit, superadmin):
    svc = _svc(db, scope_for(community.id), superadmin)
    inv = await _invoice(svc, unit)
    await svc.post_invoice(inv.id)
